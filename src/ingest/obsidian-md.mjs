...
export function ingestObsidianDecisions(archivePath, projeId = "unknown") {
  const dir = join(archivePath, "4-obsidian-vault", "projects", projeId, "decisions");
  const out = [];
  for (const file of listMd(dir)) {
    const name = basename(file);
    const sourcePath = `4-obsidian-vault/projects/${projeId}/decisions/${name}`;
...
export function ingestObsidianBugs(archivePath, projeId = "unknown") {
  const dir = join(archivePath, "4-obsidian-vault", "projects", projeId, "bugs");
  const out = [];
  for (const file of listMd(dir)) {
    const name = basename(file);
    const sourcePath = `4-obsidian-vault/projects/${projeId}/bugs/${name}`;
    const r = readAndParse(file, sourcePath);
    if (r.error) {
      out.push({ source: { kind: "obsidian", path: sourcePath }, error: r.error, raw_excerpt: r.raw_text_excerpt });
      continue;
...
export function ingestObsidianSprints(archivePath, projeId = "unknown") {
  const dir = join(archivePath, "4-obsidian-vault", "projects", projeId, "sprints");
  const out = [];
  for (const file of listMd(dir)) {
    const name = basename(file);
    const sourcePath = `4-obsidian-vault/projects/${projeId}/sprints/${name}`;
    const r = readAndParse(file, sourcePath);
    if (r.error) {
...
export function listObsidianRootDocs(archivePath, projeId = "unknown") {
  ...
  // <proje>/ kok belgeleri
  const projectRoot = join(dir, "projects", projeId);
  if (existsSync(projectRoot)) {
    for (const f of ["STATE.md","project.md","tech.md","index.md"]) {
      if (existsSync(join(projectRoot, f))) out.push(`4-obsidian-vault/projects/${projeId}/${f}`);
    }
  }
  return out;
}
